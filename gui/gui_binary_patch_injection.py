...
def CreateBinaryFileInjectionGui(current_project_data: ProjectData):
    with dpg.tab(label="Binary Patch Injection", tag="binary_patch_injection_tab"):

        dpg.add_text("Binary Patches:")
        dpg.add_listbox(items=[], tag="binary_patches_listbox", callback=callback_binary_patch_selected, user_data=current_project_data, num_items=5)

        # Right-click context menu for the listbox
        with dpg.popup(parent="binary_patches_listbox", mousebutton=dpg.mvMouseButton_Right):
            dpg.add_menu_item(label="Rename Binary Patch", callback=callback_show_rename_binary_patch_popup, user_data=current_project_data)

        # Buttons for binary patches
        with dpg.group(horizontal=True):
            dpg.add_button(tag="add_new_binary_patch_button", label="Add New Binary Patch", callback=callback_show_add_binary_patch_popup, user_data=current_project_data)
            dpg.add_button(tag="remove_selected_binary_patch_button", label="Remove Selected Binary Patch", callback=callback_remove_binary_patch, user_data=current_project_data)

        dpg.add_separator()
        dpg.add_spacer(height=10)

        # Binary Patch Details
        dpg.add_text("Binary Patch Name") 
        with dpg.group(horizontal=True):
            dpg.add_input_text(tag="binary_patch_name_detail_input", hint="Name of selected binary patch", enabled=False)
        ...
        dpg.add_text(f"Binary Files")
        dpg.add_listbox([], tag="binary_patch_files_listbox", callback=callback_binary_file_selected, user_data=current_project_data, num_items=6)

        # New Binary Patch
        with dpg.group(horizontal=True):
            dpg.add_button(tag="add_binary_patch_file_button", label="Add Binary File", callback=callback_add_binary_file, user_data=current_project_data)
            dpg.add_button(label="Remove Selected Binary File", tag="remove_selected_binary_patch_file_button", callback=callback_remove_selected_binary_file, user_data=current_project_data)
            dpg.add_button(label="Clear Binary Files List", tag="clear_binary_patch_files_button", callback=callback_clear_binary_files, user_data=current_project_data)
        ...
        with dpg.group(horizontal=True, tag="binary_patch_address_in_memory_group"):
            dpg.add_text("0x")
            dpg.add_input_text(tag="binary_patch_address_in_memory", hint="80123456", hexadecimal=True,
        ...
        with dpg.group(horizontal=True):
            dpg.add_text("0x")
            dpg.add_input_text(tag="binary_patch_injection_file_address_input", hint="1000", hexadecimal=True)
            dpg.add_checkbox(label="Auto-Calculate from Offset", tag="binary_patch_auto_calc_file_address_checkbox",
                             callback=callback_auto_calculate_file_address_checkbox, user_data=current_project_data)

        # Optional options (figure out way to make this more clear later. Probably shouldn't have seperated the 3 modification types into fully seperate things since they share so much in common.)
        
        # Region Size
        dpg.add_text("Size of Region")
        with dpg.group(horizontal=True):
            dpg.add_text("0x")
            dpg.add_input_text(tag="binary_patch_size_of_region", hint="1234", hexadecimal=True)
...
def BinaryPatchInjectionChangeGameFiles(items): 
    dpg.configure_item("binary_patch_target_game_file", items=items)

def ClearGuiBinaryPatchData():
    ...
    _currently_selected_binary_patch_name = None
    _currently_selected_binary_patch_index = -1
    verbose_print("GUI Binary Patch Data Cleared.") 


def callback_show_add_binary_patch_popup(sender, app_data, user_data): 
    global _new_binary_patch_name_input_value
    _new_binary_patch_name_input_value = "" 

    if dpg.does_item_exist("add_binary_patch_modal"):
        dpg.set_value("new_binary_patch_name_input", _new_binary_patch_name_input_value)
        dpg.show_item("add_binary_patch_modal")
    else:
        ...
        with dpg.window(label="Add New Binary Patch", modal=True, no_close=True, tag="add_binary_patch_modal", 
                        no_move=True, no_resize=True, width=modal_width, height=modal_height, pos=[modal_x, modal_y], show=False):
            dpg.add_text("Enter name for the new binary patch:") 
            dpg.add_input_text(tag="new_binary_patch_name_input", default_value=_new_binary_patch_name_input_value, width=200, hint="New Binary Patch Name")
            dpg.add_spacer(height=10)
            with dpg.group(horizontal=True):
                dpg.add_button(label="Create", callback=callback_create_binary_patch_from_popup, user_data=user_data) 
                dpg.add_button(label="Cancel", callback=lambda s,a,u: dpg.configure_item("add_binary_patch_modal", show=False))
        dpg.show_item("add_binary_patch_modal")


def callback_create_binary_patch_from_popup(sender, app_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_name
    global _currently_selected_binary_patch_index

    new_binary_patch_name = dpg.get_value("new_binary_patch_name_input")

    if not new_binary_patch_name:
        messagebox.showerror("Error", "Binary Patch name cannot be empty.") 
        return

    if new_binary_patch_name in current_project_data.GetCurrentBuildVersion().GetBinaryPatchNames():
        messagebox.showerror("Error", f"A binary patch named '{new_binary_patch_name}' already exists.") 
        return

    new_binary_patch = BinaryPatch()
    ...
    if main_exe:
        new_binary_patch.SetInjectionFile(main_exe)
        
    new_binary_patch.SetAutoCalculateInjectionFileAddress(True)

    current_project_data.GetCurrentBuildVersion().AddBinaryPatch(new_binary_patch)
    print(f"Created new binary patch: {new_binary_patch_name}") 
    ...
    dpg.set_value("binary_patches_listbox", new_binary_patch_name)

    ReloadGuiBinaryPatchData(current_project_data) 

    dpg.configure_item("add_binary_patch_modal", show=False)
    
    from gui.gui_main_project import trigger_auto_save
    trigger_auto_save()


def callback_show_rename_binary_patch_popup(sender, app_data, current_project_data: ProjectData): 
    global _rename_binary_patch_name_input_value
    global _currently_selected_binary_patch_name

    if _currently_selected_binary_patch_index == -1:
        messagebox.showinfo("Info", "No binary patch selected to rename.") 
        return

    _rename_binary_patch_name_input_value = _currently_selected_binary_patch_name

    if dpg.does_item_exist("rename_binary_patch_modal"):
        dpg.set_value("rename_binary_patch_name_input", _rename_binary_patch_name_input_value)
        dpg.show_item("rename_binary_patch_modal")
    else:
        viewport_width = dpg.get_viewport_width()
        viewport_height = dpg.get_viewport_height()
        modal_width = 350
        ...
        with dpg.window(label="Rename Binary Patch", modal=True, no_close=True, tag="rename_binary_patch_modal", 
                        no_move=True, no_resize=True, width=modal_width, height=modal_height, pos=[modal_x, modal_y], show=False):
            dpg.add_text(f"Rename '{_currently_selected_binary_patch_name}':") 
            dpg.add_input_text(tag="rename_binary_patch_name_input", default_value=_rename_binary_patch_name_input_value, width=200, hint="New Binary Patch Name")
            dpg.add_spacer(height=10)
            with dpg.group(horizontal=True):
                dpg.add_button(label="Rename", callback=callback_rename_binary_patch_from_popup, user_data=current_project_data) 
                dpg.add_button(label="Cancel", callback=lambda s,a,u: dpg.configure_item("rename_binary_patch_modal", show=False))
        dpg.show_item("rename_binary_patch_modal")


def callback_rename_binary_patch_from_popup(sender, app_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_name

    new_name = dpg.get_value("rename_binary_patch_name_input")
    old_name = _currently_selected_binary_patch_name

    if not new_name:
        messagebox.showerror("Error", "New binary patch name cannot be empty.") 
        return

    if new_name == old_name:
        dpg.configure_item("rename_binary_patch_modal", show=False)
        return

    if new_name in current_project_data.GetCurrentBuildVersion().GetBinaryPatchNames(): 
        messagebox.showerror("Error", f"A binary patch named '{new_name}' already exists.") 
        return

    selected_binary_patch_obj = current_project_data.GetCurrentBuildVersion().GetBinaryPatches()[_currently_selected_binary_patch_index] 
    selected_binary_patch_obj.SetName(new_name)

    _currently_selected_binary_patch_name = new_name
    UpdateBinaryPatchesListbox(current_project_data) 
    dpg.set_value("binary_patch_name_detail_input", new_name)

    print(f"Binary Patch renamed from '{old_name}' to '{new_name}'.") 
    dpg.configure_item("rename_binary_patch_modal", show=False)
...
def callback_add_binary_file(sender, button_data, current_project_data: ProjectData):
    ...
    if not does_file_already_exist:
        selected_binary_patch.AddBinaryFile(code_file_path)
...
def callback_remove_selected_binary_file(sender, button_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    if _currently_selected_binary_patch_index == -1:
        messagebox.showerror("Error", "No binary patch selected.") 
        return

    selected_item_value = dpg.get_value("binary_patch_files_listbox")
    if not selected_item_value:
        messagebox.showinfo("Info", "No binary file selected in the listbox to remove.") 
        return
    ...
    try:
        ...
        selected_binary_patch.code_files.pop(idx_in_displayed_names) 
        UpdateBinaryFilesListbox(current_project_data) 
        print(f"Removed binary file: {selected_item_value}") 
    except ValueError:
        messagebox.showerror("Error", f"Could not find '{selected_item_value}' in the current binary patch's files.") 
    except IndexError:
        messagebox.showerror("Error", "Selected binary patch data is out of sync. Please re-select a binary patch.") 
    
    from gui.gui_main_project import trigger_auto_save
    trigger_auto_save()

def callback_clear_binary_files(sender, button_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    if _currently_selected_binary_patch_index == -1:
        messagebox.showerror("Error", "No binary patch selected to clear files from.") 
        return

    if messagebox.askyesno("Confirm Clear", "Are you sure you want to clear all binary files for the current binary patch?"): 
        selected_binary_patch = current_project_data.GetCurrentBuildVersion().GetBinaryPatches()[_currently_selected_binary_patch_index] 
        selected_binary_patch.code_files = [] # Assuming code_files is a list
        UpdateBinaryFilesListbox(current_project_data) 
        print("Cleared all binary files for the current binary patch.") 
    
    from gui.gui_main_project import trigger_auto_save
    trigger_auto_save()

def callback_binary_file_selected(sender, app_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    if _currently_selected_binary_patch_index != -1 and app_data:
        current_binary_patch_obj = None
        if 0 <= _currently_selected_binary_patch_index < len(current_project_data.GetCurrentBuildVersion().GetBinaryPatches()): 
...
def callback_binary_patch_selected(sender, data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_name
    global _currently_selected_binary_patch_index

    selected_display_name = dpg.get_value("binary_patches_listbox")
    ...
    found_index = -1
    for i, binary_patch_name in enumerate(current_project_data.GetCurrentBuildVersion().GetBinaryPatchNames()): 
    ...
    if found_index != -1:
        ...
        verbose_print(f"Binary Patch selected: {_currently_selected_binary_patch_name} (Index: {_currently_selected_binary_patch_index})") 
        ReloadGuiBinaryPatchData(current_project_data) 
    else:
        print(f"Error: Selected binary patch '{_currently_selected_binary_patch_name}' not found in project data. Clearing GUI.") 
        ClearGuiBinaryPatchData() 


def GetGuiBinaryPatchData(current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    temp_binary_patch = BinaryPatch()

    binary_patch_name = dpg.get_value("binary_patch_name_detail_input")
    if not binary_patch_name: # Check for empty string
        messagebox.showerror("Validation Error", "Binary Patch name cannot be empty.") 
        return None
    ...
    game_file = dpg.get_value("binary_patch_target_game_file")
    temp_binary_patch.SetInjectionFile(game_file)

    memory_address = dpg.get_value("binary_patch_address_in_memory")
    ...
    auto_calc_enabled = dpg.get_value("binary_patch_auto_calc_file_address_checkbox")
    temp_binary_patch.SetAutoCalculateInjectionFileAddress(auto_calc_enabled)

    if auto_calc_enabled:
        injection_file_address = dpg.get_value("binary_patch_injection_file_address_input")
    ...
    size_of_region = dpg.get_value("binary_patch_size_of_region")
...
def callback_binary_patch_disabled_checkbox_changed(sender, app_data, user_data):
    ...
    # True = disabled, False = enabled. (Backwards is confusing here, but I think having a checkbox for "disabled" makes more sense than everything having a checked "enabled" box)
    binary_patch.SetEnabled(not app_data)

    # Update listbox to show [DISABLED] prefix
    UpdateBinaryPatchesListbox(current_project_data)

    # Keep the current binary patch selected
...
def UpdateBinaryPatchesListbox(current_project_data: ProjectData): 
    ...
    dpg.configure_item("binary_patches_listbox", items=binary_patch_display_names)


def UpdateBinaryFilesListbox(current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    if _currently_selected_binary_patch_index != -1:
        selected_binary_patch = current_project_data.GetCurrentBuildVersion().GetBinaryPatches()[_currently_selected_binary_patch_index] 
        code_files_names = [os.path.basename(f) for f in selected_binary_patch.GetCodeFilesPaths()]
        dpg.configure_item("binary_patch_files_listbox", items=code_files_names)
    else:
        dpg.configure_item("binary_patch_files_listbox", items=[])
...
def callback_save_binary_patch(sender, button_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_index

    if _currently_selected_binary_patch_index == -1:
        messagebox.showerror("Error", "No binary patch selected to save.") 
        return

    temp_binary_patch_from_gui = GetGuiBinaryPatchData(current_project_data) 
    ...
    print(f"Binary Patch '{existing_binary_patch.GetName()}' details saved.") 
...
def callback_remove_binary_patch(sender, button_data, current_project_data: ProjectData): 
    global _currently_selected_binary_patch_name
    global _currently_selected_binary_patch_index

    selected_binary_patch_name = dpg.get_value("binary_patches_listbox")

    if not selected_binary_patch_name:
        messagebox.showinfo("Info", "No binary patch selected to remove.") 
        return

    if messagebox.askyesno("Confirm Removal", f"Are you sure you want to remove binary patch '{selected_binary_patch_name}'?"): 
        binary_patch_to_remove_index = None
        for i, project_binary_patch_name in enumerate(current_project_data.GetCurrentBuildVersion().GetBinaryPatchNames()): 
        ...
        if binary_patch_to_remove_index is not None:
            ...
            UpdateBinaryPatchesListbox(current_project_data) 
            ClearGuiBinaryPatchData() 
            print(f"Removed binary patch: {selected_binary_patch_name}") 
        else:
            messagebox.showerror("Error", f"Binary Patch '{selected_binary_patch_name}' not found in project data.") 
...
def callback_auto_calculate_file_address_checkbox(sender, app_data, current_project_data: ProjectData):
    ...
    if _currently_selected_binary_patch_index == -1:
        messagebox.showinfo("Info", "No binary patch selected. Cannot change auto-calculate setting.") 
        dpg.set_value("binary_patch_auto_calc_file_address_checkbox", False)
        dpg.configure_item("binary_patch_injection_file_address_input", enabled=True)
        return
    ...
    dpg.configure_item("binary_patch_injection_file_address_input", enabled=not is_checked)
