...
class CompilationService:
    ...
    def _compile_sources(self) -> CompilationResult:
        ...
        # Determine which files need compilation (incremental build check)
        files_to_compile = []
        compiled_obj_files = []
        files_skipped = 0
        ...
        for src_file_path in all_source_files:
            ...
            if should_compile:
                files_to_compile.append((src_file_path, obj_file_path, reason))
            else:
        ...
        if len(files_to_compile) == 0:
            ...
            for src_file_path, obj_file_path, reason in files_to_compile:
                ...
                compile_result = self._compile_single_file(src_file_path, obj_output_dir, platform)
            ...
            # Many files (3+) - compile in parallel
            max_workers = min(multiprocessing.cpu_count(), len(files_to_compile))

            # Display files being compiled
            for src_file_path, _, reason in files_to_compile:
                src_filename = os.path.basename(src_file_path)
                compile_reason = f" ({reason})" if reason and self.verbose else ""
                self._log_progress(f"  {src_filename}{compile_reason}")
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_file = {
                    executor.submit(self._compile_single_file, src_path, obj_output_dir, platform): (src_path, obj_path)
                    for src_path, obj_path, _ in files_to_compile
                }

                # Process results as they complete
                for future in as_completed(future_to_file):
                    src_file_path, obj_file_path = future_to_file[future]
                    src_filename = os.path.basename(src_file_path)
                    obj_filename = os.path.splitext(src_filename)[0] + '.o'
                    ...
                    try:
                        ...
                        result.message = f"Compilation error for {src_filename}"
    ...
    def _compile_single_file(self, src_file_path: str, output_dir: str, platform: str) -> CompilationResult:
        ...
        cpp_extensions = ('.cpp', '.cc', '.cxx', '.c++', '.CPP', '.CXX', '.CC', "C++")
        is_cpp_file = src_file_path.lower().endswith(cpp_extensions)
            ...
            for line in lines:
                if "overflowed by" in line:
                    ...
                    if result is not None:
                        # Parse format: "region `.CODECAVE_MyCodecave' overflowed by 0x123 bytes"
                        try:
                            # Extract section name (between backticks or "region " and " overflowed")
                            if "`" in line:
                            ...
                            try:
                                # Parse as integer (handles both hex and decimal)
                            ...
                            # If parsing fails, just skip structured extraction
                            pass
    ...
    def _extract_sections(self) -> CompilationResult:
        ...
        num_sections = len(sections_to_extract)
        if self.verbose:
        ...
        try:
            ...
            for section_name in sections_to_extract:
                ...
                if self.verbose:
                    self._log_progress(f"    {output_bin_name}")
            ...
            result.success = False
            result.details = "objcopy timed out after 10 seconds"
            self._log_error(result.details)
        except FileNotFoundError:
            result.success = False
    ...
    def _display_warnings_block(self, filename: str, warnings_text: str):
        ...
        if self.on_progress:
            self.on_progress("")
            self.on_progress("╔══════════════════════════════════════════════════════════╗")
            self.on_progress(f"║            COMPILER WARNINGS ({filename})".ljust(61) + "║")
            self.on_progress("╚══════════════════════════════════════════════════════════╝")
