...
def InjectIntoDolphin(project_path, codecaves, hooks):
    ...
    dolphin_pid = get_pid(process_name)
    if dolphin_pid is None:
        exit_message = colored("Could not find Dolphin Process!", "red")
        print(exit_message)
        return "Could not find Dolphin Process!"
        
    dolphin_handle = ctypes.windll.kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, dolphin_pid)
    ...
    #Get dolphin base address from pointer
    base_address = GetBaseAddressFromEXE(dolphin_handle, "Dolphin.exe")
    
    ptr_address = read_process_memory(dolphin_handle, ctypes.c_ulonglong(base_address + MAIN_RAM_PTR_OFFSET), 8)
    pointer_value = int.from_bytes(ptr_address, byteorder='little', signed=False)  
    MAIN_RAM = pointer_value    
    print(colored(f"Dolphin base RAM value: {hex(MAIN_RAM)}", "blue"))   
    ...
    for cave in codecaves:
        ...
        change_memory_protection(dolphin_handle, address_to_write_to, len(mod_data[cave[0]]), PAGE_EXECUTE_READWRITE)
        if write_to_process_memory(dolphin_handle, address_to_write_to, mod_data[cave[0]], len(mod_data[cave[0]])):
            print(colored(f"Successfully placed custom function code at: {hex(int(cave[1], base=16))}", "blue"))
        else:
            error_code = ctypes.windll.kernel32.GetLastError()
    ...
    for hook in hooks:
        ...
        change_memory_protection(dolphin_handle, address_to_write_to, len(mod_data[hook[0]]), PAGE_EXECUTE_READWRITE)
        if write_to_process_memory(dolphin_handle, address_to_write_to, mod_data[hook[0]], len(mod_data[hook[0]])):
            print(colored(f"Successfully placed custom hook code at: {hex(int(hook[1], base=16))}", "blue"))
        

    # Close the process handle
    ctypes.windll.kernel32.CloseHandle(dolphin_handle)
    
    print(colored(f"Successfully injected mod into dolphin!", "yellow"))
    return "Successfully injected mod into dolphin!"
...
if __name__ == "__main__":
    ...
    InjectIntoDolphin(project_path, code_caves, hooks)