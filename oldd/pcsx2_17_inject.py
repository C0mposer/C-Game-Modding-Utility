...
def InjectIntoPCSX2(project_path, codecaves, hooks):
    ...
    if pcsx2_pid is None:
        exit_message = colored("Could not find PCSX2 1.7.5112 Process!", "red")
        print(exit_message)
        return "Could not find PCSX2 1.7.5112 Process!"
        
    pcsx2_handle = ctypes.windll.kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, pcsx2_pid)
    
    base_address = GetBaseAddressFromEXE(pcsx2_handle, "pcsx2.exe")
    ...
    #Get pcsx2 base address from pointer
    ptr_address = read_process_memory(pcsx2_handle, ctypes.c_ulonglong(base_address + MAIN_RAM_PTR_OFFSET), 8)
    try:
    ...
    print(colored(f"PCSX2 1.7.5112 base RAM value: {hex(MAIN_RAM)}", "blue"))   
    ...
    print(colored(f"Successfully injected mod into pcsx2!", "yellow"))
    return "Successfully injected mod into PCSX2!"

def Log(project_path, codecaves, hooks):
    cProfile.runctx('InjectIntoPCSX2(project_path, codecaves, hooks)', {'project_path': project_path, 'codecaves': codecaves, 'hooks': hooks, "InjectIntoPCSX2": InjectIntoPCSX2}, {})
